...
const PrivateSector = () => {
  ...
  // ✅ show alert after clicking connect
  const handleConnection = async (company) => {
    setLoadingConnections((prev) => ({ ...prev, [company.user_id]: true }));
    try {
      ...
      if (data.success) {
        alert(
          `You have connected to ${company.company_name} Private Sector successfully`
        );
        setConnectedCompanies((prev) => ({
          ...prev,
          [company.user_id]: true,
        }));
      }
    } catch (error) {
      ...
      setLoadingConnections((prev) => ({ ...prev, [company.user_id]: false }));
    }
  };
